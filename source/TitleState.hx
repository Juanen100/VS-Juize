...
class TitleState extends MusicBeatState
{
	...
	var curWackyEngli:Array<String> = [];
	...
	override public function create():Void
	{
		...
		curWackyEngli = FlxG.random.getObject(getIntroTextShitEn());
		...
		#else
		if(!FlxG.save.data.skipIntro && !Intro.leftState) {
	...
	function startIntro()
	{
		...
		gfDance.animation.addByIndices('danceLeft', 'juizeTitle instancia 1', [0, 1, 2, 3, 4, 5, 6], "", 24, false);
		gfDance.animation.addByIndices('danceRight', 'juizeTitle instancia 1', [7, 8, 9, 10, 11, 12, 13], "", 24, false);
		...
		// add(logo);
		// "add(logo);" was for the old logo (Don't know why Kade Dev left it on the source)

		//Don't know this is btw
	...
	function getIntroTextShitEn():Array<Array<String>>
	{
		//Shitty fuction translated for the fucking language system

		var fullText:String = Assets.getText(Paths.txt('introTextEn'));
	...
	override function beatHit()
	{
		...
		if (FlxG.save.data.language)
		{
			switch (curBeat)
			{
				...
				case 2:
					...
					if (Main.watermarks)
						createCoolText(['Juizepanka', 'EdgarXammar', 'Juanen 100', 'SamuramongeDev', 'BitFoxOriginal', 'ItsNexxos']);
					else
					{
				...
				case 5:
					if (Main.watermarks)
						createCoolText(['Kade Engine', 'por']);
					else
						createCoolText(['En colaboración', 'con']);
				case 7:
					if (Main.watermarks)
		...
		else
		{
			switch (curBeat)
			{
				...
				// credTextShit.screenCenter();
				case 9:
					createCoolText([curWackyEngli[0]]);
				// credTextShit.visible = true;
				case 10:
					addMoreText(curWackyEngli[1]);
				// credTextShit.text += '\nlmao';
				case 11:
					deleteCoolText();
				// credTextShit.visible = false;
